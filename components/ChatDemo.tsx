...
export default function ChatDemo() {
  ...
  // Play current scenario then cycle to next
  useEffect(() => {
    if (!started) return
  ...
  return (
    <div ref={ref} className="bg-[#111118] rounded-2xl border border-white/10 p-5 w-full max-w-sm shadow-2xl">
      ...
      {/* Messages — fixed height, auto-scrolls to latest, scrollbar hidden */}
      <div
        ref={msgsRef}
        className="space-y-3 h-[320px] overflow-y-auto scroll-smooth transition-opacity duration-300"
        style={{ opacity: fading ? 0 : 1, scrollbarWidth: 'none', msOverflowStyle: 'none' } as React.CSSProperties}
      >
